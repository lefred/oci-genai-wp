...
def answer_user_question(query):
    ...
    with connectMySQL(myconfig) as db:
        ...
        llm_response_result = query_llm_with_prompt(similar_docs, prompt)
        response = {}
...
if cnx.is_connected():
    ...
    if myanswer["text"]["data"].chat_response.documents is not None:
        for el in myanswer["text"]["data"].chat_response.documents:
            cursor.execute(
                "SELECT post_title from wp_posts where id = {}".format(el["id"])
            )
            result = cursor.fetchone()
            print(" - [{}]: {}".format(el["id"], result[0]))
    else:
        print("no documents pointed in the response or too long")
    cnx.close()
